...
<html >
	<body>
		<div class="container marginbot-50 home-section color-dark">	
			<h3 class="h-bold">Admin Dashboard 
				<span class="pull-right fa fa-user-circle"> <?php $user = $this->request->session()->read('Auth.User');
					if(!empty($user)) {
    				echo $user['email'] ;
					}
				?> </span><br/></h3>
				<?= $this->Html->link(__('View Profile'), ['controller' => 'Admin','action' => 'adminprofile', $user['id']], array('class'=>'btn btn-lg btn-primary pull-right')); ?></h3>
